...
from models import resnet, resnext, repvgg, wideresnet, sk_net, small_cnn, se_resnet, cbam_resnet, danet, epsanet
from loss import st
from loss import at
...
parser.add_argument('--model-dir', default='../model-cifar-pgd',
...
transform_train = transforms.Compose([
    ...
    transforms.RandomApply(trans_random, p=0.7),
    transforms.ToTensor(),
...
def getMethod(model, x_natural, y, optimizer, step_size, epsilon, perturb_steps, beta, distance):
    ...
    if lossFuc == 'mart':
        MyLoss = mart.mart_pgd_loss(model, x_natural, y, optimizer, step_size, epsilon, perturb_steps, beta, distance)
    if lossFuc == 'trades':
        MyLoss = trades.trades_pgd_loss(model, x_natural, y, optimizer, step_size, epsilon, perturb_steps, beta,
                                        distance)
    if lossFuc == 'rwat':
        MyLoss = rwat.rwat_pgd_loss(model, x_natural, y, optimizer, step_size, epsilon, perturb_steps, distance)
    if lossFuc == 'pwat':
        MyLoss = pwat.pwat_pgd_loss(model, x_natural, y, optimizer, step_size, epsilon, perturb_steps, distance)
    if lossFuc == 'at':
        MyLoss = at.at_pgd_loss(model, x_natural, y, optimizer, step_size, epsilon, perturb_steps, distance)
    return MyLoss
...
def main():
    ...
    print('PGD:The log used the model-{},and used the loss-{}'.format(args.model, args.loss))
    ...
    getPic(xTick, cleanAcu, fgsmAcu, pgdAcu, deepfoolAcu, cwAcu, 71, 1.01, args.model, args.loss, attack='pgd')


if __name__ == '__main__':
    torch.cuda.empty_cache()
    sys.stdout = Logger(sys.stdout, args.model, args.loss, attack='pgd')  # 将输出记录到log
    sys.stderr = Logger(sys.stderr, args.model, args.loss, attack='pgd')  # 将错误信息记录到log
    main()

...
def rwat_fgsm_loss(model,
                   ...
                   distance='l_inf'):
    model.eval()
    weight = torch.rand(x_natural.shape)
    ...
    loss = 0.2 * F.cross_entropy(model(x_natural), y) + 0.4 * F.cross_entropy(model(x_radv), y) + 0.4 * F.cross_entropy(
        model(x_adv), y)
    return loss


def rwat_pgd_loss(model,
                  x_natural,
                  y,
                  optimizer,
                  step_size,
